...
public class LentDAOImpl implements LentDAO {
	...
	public List<Map<String, Object>> selectLentList(Map<String, Object> lent) {
		...
		try {
			con = Connector.open();
			String sql = "select l.*,m.m_name,b.b_title from lent l, member m ,book b where l.m_num=m.m_num and b.b_num=l.b_num";
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public List<Map<String, Object>> selectNoLentBookList() {
		...
		try {
			con = Connector.open();
			String sql = "select b_num,b_title from book where b_num not in(select b_num from lent where l_recdate is null)";
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
				Map<String, Object> map = new HashMap<>();
